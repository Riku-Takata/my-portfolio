...
const DeskAnimation: React.FC = () => {
  ...
  const timeoutIdRef = useRef<NodeJS.Timeout | null>(null) // useRef を使用して値を保持
  ...
  useEffect(() => {
    ...
    function typeText() {
      if (!codeElement) return
      if (index === 0) codeElement.textContent = "" // ★ ここでリセット

      if (index < text.length) {
        codeElement.textContent += text.charAt(index)
        index++
        timeoutIdRef.current = setTimeout(typeText, 100) // Adjust typing speed here
      } else {
    ...
    return () => {
      if (timeoutIdRef.current) clearTimeout(timeoutIdRef.current) // ★ useEffect のクリーンアップ
    }
  }, [])
